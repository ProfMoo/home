...
A mono-repository for my homelab Kubernetes cluster. I strictly adhere to Infrastructure as Code (IaC) and GitOps practices using OSS tools like Kubernetes, OpenTofu, Talos, Flux, Renovate, and GitHub Actions.
...
I use [Talos](https://github.com/siderolabs/talos), [OpenTofu](https://opentofu.org/), and [Proxmox](https://github.com/proxmox) to spin up Kubernetes in a GitOps fashion in [this directory](./infrastructure).

Proxmox, a VM-management and hypervisor technology, is used to spin up VMs on the compute bare-metal rack servers. These raw VMs are bootstrapped via OpenTofu with Talos configuration(s) that create a functional Kubernetes cluster with the initial cluster components (such as Flux) already deployed.
...
<details>
  ...
  <img src="./docs/rack.jpg" align="center" width="550px" alt="rack" />
</details>
...
| [SuperMicro SYS-6028U-TR4T+](https://www.supermicro.com/en/products/system/2u/6028/sys-6028u-tr4t_.php)   | 56   |  1x512GB SSD      | 2x512GB SSD (k8s nodes) / 1x512GB SSD (`rook-ceph`) | 126GB | Proxmox         | Compute              |
| [SuperMicro SYS-6028U-TR4T+](https://www.supermicro.com/en/products/system/2u/6028/sys-6028u-tr4t_.php)   | 56   |  1x512GB SSD      | 2x512GB SSD (k8s nodes) / 3x512GB SSD (`rook-ceph`) | 256GB | Proxmox         | Compute              |
| Dell PowerEdge R530         |  20  | 1x512GB SSD      | 2x512GB SSD (k8s nodes) / 4x512GB SSD (`rook-ceph`)     | 156GB | Proxmox | Compute     |
...
| [TrueNas Mini R](https://www.truenas.com/blog/meet-the-mini-r/)         | 8   | Internal SATA DOMs      | 6x22TB HDD     | 32GB | TrueNAS Scale | Storage     |
| UniFi USP PDU Pro           | -   | -            | -                               | -    | -             | PDU                     |
| [Eaton 5PX1500RT - 1500VA](https://www.eaton.com/us/en-us/skuPage.5PX1500RT.html)             | -  | -            | -                               | -    | -             | UPS                     |
...
Some binaries needed for local development: `claude` `talosctl` `kubectl` `flux` `tofu` (`tenv`) `docker` `yamlfmt` `markdownlint-cli2` `hadolint` `shfmt`, `minijinja-cli`
