  ...
  control_plane_configs = {
    for key, node in var.control_plane : node.name => templatefile(
      "configs/controlplane.yaml",
      {
        ...
        k8s_ca_crt                  = data.sops_file.talos_secrets.data["talos.machineconfig.certs.k8s.crt"]
        k8s_ca_key                  = data.sops_file.talos_secrets.data["talos.machineconfig.certs.k8s.key"]
        ...
        bootstrap_token             = data.sops_file.talos_secrets.data["talos.machineconfig.secrets.bootstraptoken"]
        secretbox_encryption_secret = data.sops_file.talos_secrets.data["talos.machineconfig.secrets.secretboxencryptionsecret"]
        talos_ca_crt                = data.sops_file.talos_secrets.data["talos.machineconfig.certs.os.crt"]
        talos_ca_key                = data.sops_file.talos_secrets.data["talos.machineconfig.certs.os.key"]
